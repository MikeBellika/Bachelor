...
void print_data_array(uint8_t *array, int length){
     for (unsigned n=0; n<length; n++) { printf("%02x ", array[n]); }
}
...
static int m_bus_decode_format_b(r_device *decoder, const m_bus_data_t *in, m_bus_data_t *out, m_bus_block1_t *block1)
{
    ...
    // printf("\n---------FULL DATA---------\n");
    // print_data_array(in->data, in->length);
    // printf("\n---------DATA END---------\n");
    // printf("%x", in->data[0]);
    ...
    printf("\n| \x1B[31m");
    print_data_array(in->data, BLOCK1B_SIZE);
    printf("\x1B[0m| \x1B[36m");
    print_data_array(in->data+BLOCK1B_SIZE, block1->L-BLOCK1B_SIZE);
    printf("\x1B[0m|");
    ...
    // printf("\n--------------BLOCK2:-----------\n");
    // print_data_array(in->data+BLOCK1B_SIZE, (MIN(block1->L-11, BLOCK2B_SIZE-2)));
    // printf("\n-------------\n");
	...
	printf("-----------BLOCK3:------------\n");
	print_data_array(in->data+BLOCK1B_SIZE+BLOCK2B_SIZE, block1->L-L_OFFSET-2);
	printf("------------------------------\n");
